...
check_convergence(my_model)

# Compare models ----------------------------------------------------------
this_model = my_model
this_name = deparse(substitute(my_model))

# Compare Recruits time series:
ss_rec = SS_reportEM$timeseries[SS_reportEM$timeseries$Yr >= min_year & SS_reportEM$timeseries$Yr <= max_year, 'Recruit_0']
wham_rec = this_model$rep$NAA[,1] # Estimated values
plot(ss_rec, type = 'l', ylim = c(0,2e+06))
lines(wham_rec, col = 2)
