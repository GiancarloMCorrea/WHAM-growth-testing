...
# Run model:
my_model_2 = fit_wham(my_input, do.osa = FALSE, do.fit = FALSE)
...
# Check mean length at age estimates (for the stock) -------------------------------------------
data_plot = data.frame(ages = 1:n_ages, mlen = this_model$rep$mLAA[1,])
p2 = ggplot(data = data_plot, aes(ages, mlen)) +
  geom_line() +
  geom_point() +
  xlab('Ages') +
  ylab('Mean length (cm)') +
...
ggsave(paste0('images/ML-at-age_', this_name,'.png'), plot = p2, width = 110, height = 80, units = 'mm', dpi = 300)

# Check lengths at age estimates (for the stock) -------------------------------------------
png(filename = paste0('images/prop-len-at-age_', this_name,'.png'), res = 300, width = 90, height = 90, units = 'mm')
par(mar = c(4,4,0.5,0.5))
matplot(this_model$rep$LAA[40,,], type = 'l', xlab = 'lengths', ylab = 'proportion')
dev.off()
...
# Check hessian -----------------------------------------------------------
my_hessian = this_model$he()

png(filename = paste0('images/hessian_', this_name,'.png'), width = 120, height = 90, units = 'mm', res = 300)
par(mar=c(3,2,1,5))
image(my_hessian,axes=F,col='transparent')
axis(1, at = c(0 , 1), labels = c(1, ncol(my_hessian)))
axis(2, at = c(0 , 1), labels = c(1, nrow(my_hessian)))
image.plot(my_hessian, add=T, legend.mar = 5)
dev.off()
